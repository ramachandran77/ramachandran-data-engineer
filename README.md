# Hi there 👋 I'm Ramachandran

## Senior Data Engineer | PySpark Developer | ETL Architect | Trading Analytics Engineer

Experienced IT professional with 18+ years of expertise in Data Engineering, ETL Development, Data Warehousing, Big Data Ecosystems, and Python-based Analytics Solutions.

I specialize in designing scalable enterprise-grade ETL pipelines, distributed data processing frameworks, cloud-ready architectures, and real-time analytics platforms.

## 🚀Project Experience:
## ERP Data Analytics:
SQL – Data Exploration and Mining, Power BI – Dashboard.
	•	Analysed sales data of a hardware manufacturing company.
	•	Generated insights related to finance, sales, market, and supply chain analytics.
	•	Used Snowflake schema format and ERD in MySQL to establish relationships between tables.
	•	Utilised statistical functions for data aggregation and summarization.
	•	Developed Stored Procedures for complex queries.
	•	Established a pipeline to derive Key Metrics.
	•	Generated reports on top markets, platforms, and customers.
	•	Utilised metrics such as Net Error, Absolute Net Error, and Forecast Accuracy.
	•	Connected Power BI to MySQL and Excel.
	•	Transformed data using Power Query and Data Modelling.
	•	Established relations by snowflake schema.
	•	Validated data against benchmark values.
	•	Utilised DAX to create calculated columns and measures.
	•	Built a dynamic dashboard with KPI’s, trends, and Top Performers.
	•	Published a report on Power BI service for UAT and Data validation through Excel Analyse.
	•	Incorporated stakeholder feedback to create an Executive Dashboard.
	•	Resolved quality issues and optimised dashboard performance.
	•	Deployed the dashboard to Power BI service with gateway setup to MySQL Database and local Excel files for Automatic Data Refresh.
	•	Utilised Project Management Skills such as Project charter, stakeholder mapping analysis, and Kanban board for task assignment.
	•	Designed a dashboard with up to three levels of analysis.
	•	Asked stakeholders many “why’s” to improve overall business.

---

## 🚀 Core Expertise

- PySpark & Spark SQL
- Cloudera CDP Ecosystem
- Hive / Impala / HDFS
- ETL & Data Warehousing
- Data Modeling
- Python Automation
- Linux Shell Scripting
- MySQL & SQL Optimization
- Plotly Dashboards
- Trading Analytics
- Nifty50 Signal Engineering
- CPR / EMA Strategies
- Docker & CI/CD
- Airflow Orchestration

---

## 🛠 Tech Stack

### Big Data
![PySpark](https://img.shields.io/badge/PySpark-Advanced-orange)
![Hive](https://img.shields.io/badge/Hive-Expert-yellow)
![Impala](https://img.shields.io/badge/Impala-Advanced-blue)
![Hadoop](https://img.shields.io/badge/Hadoop-Experienced-green)

### Programming
![Python](https://img.shields.io/badge/Python-Advanced-blue)
![SQL](https://img.shields.io/badge/SQL-Expert-lightgrey)
![Linux](https://img.shields.io/badge/Linux-Advanced-black)

### DevOps & Cloud
![Docker](https://img.shields.io/badge/Docker-Intermediate-blue)
![GitHub Actions](https://img.shields.io/badge/CI/CD-GitHub_Actions-black)
![AWS](https://img.shields.io/badge/AWS-Learning-orange)

---

# 📌 Featured Projects

## 1️⃣ Enterprise PySpark ETL Framework
Production-grade ETL framework using PySpark, Hive, MySQL, Docker, and Airflow.

### Features
- Incremental ETL
- Data Validation
- Logging & Monitoring
- Retry Mechanism
- Audit Framework
- Unit Testing

---

## 2️⃣ Nifty50 Algo Trading Analytics Platform
Python-based analytics engine for intraday trading strategies using:
- CPR
- EMA Crossovers
- Support/Resistance
- Fibonacci Levels
- Plotly Interactive Charts

---

## 3️⃣ Real-Time Market Data Pipeline
Kafka + Spark Streaming + Hive pipeline for real-time analytics.

---

# 📊 GitHub Stats

![GitHub stats](https://github-readme-stats.vercel.app/api?username=yourusername&show_icons=true)

---

# 📫 Contact

- LinkedIn: www.linkedin.com/in/ramachandran-sandhappan-a1408b28/
- Email: ramachandran77@gmail.com
- GitHub: https://github.com/ramachandran77/ramachandran-data-engineer

---

# 💡 Current Focus

- Advanced Data Engineering
- Real-Time Analytics
- Cloud Data Platforms
- AI-driven Trading Systems
- Modern Data Architectures
